...
for year_number in year:
    for month_number in month:
        arcpy.md.MakeNetCDFRasterLayer(
            r"C:\Users\evank\Documents\ArcGIS\Projects\Temperature\cru_ts4.04.1901.2019.frs.dat.nc\cru_ts4.04.1901.2019.frs.dat.nc",
            "frs", "lon", "lat", "frs_" + str(year_number) + "_" + str(month_number), '',
            "time '" + str(month_number) + "/01/" + str(year_number) + " 12:00:00 AM'", "BY_VALUE", "CENTER")
    # AVERAGES THE RASTERS then deletes extra rasters
    inRaster = ["frs_" + str(year_number) + "_" + month_number for month_number in month]
    # Execute Times
    leap_year = year_number/4
    if (isinstance(leap_year, int)):
        Year_FRS_Average = (Raster(inRaster[0])*31 + Raster(inRaster[1])*28 + Raster(inRaster[2])*31 +
        ...
        Year_FRS_Average = (Raster(inRaster[0])*31 + Raster(inRaster[1])*29 + Raster(inRaster[2])*31 +
                            Raster(inRaster[3])*30 + Raster(inRaster[4])*31 + Raster(inRaster[5])*30 +
                            Raster(inRaster[6])*31 + Raster(inRaster[7])*31 + Raster(inRaster[8])*30 +
                            Raster(inRaster[9])*31 + Raster(inRaster[10])*30 + Raster(inRaster[11])*31) / 366
    # Save the output
    arcpy.management.ProjectRaster(Year_FRS_Average,
                                   r"C:/Users/evank/Documents/ArcGIS/Projects/Temperature/Temperature Variable CRU/FRS/Year_FRS_Average" + str(year_number) + ".tif",
    ...
    out_point_features = (
                r"C:/Users/evank/Documents/ArcGIS/Projects/Temperature/Temperature Variable CRU/FRS/Specimen_Location_" +
                str(year_number))
    ExtractValuesToPoints(in_point_features, "Year_FRS_Average" + str(year_number) + ".tif", out_point_features, "NONE", "Value_ONLY")
# Export the saved file as a .csv file
    import numpy
    fc = (r"C:/Users/evank/Documents/ArcGIS/Projects/Temperature/Temperature Variable CRU/FRS/Specimen_Location_" +
          str(year_number) + ".shp")
    ...
    for month_number in month:
        arcpy.Delete_management("frs_" + str(year_number) + "_" + month_number)
    arcpy.Delete_management("Year_FRS_Average" + str(year_number) + ".tif")
