...
for year_number in year:
    for month_number in month:
        arcpy.md.MakeNetCDFRasterLayer(
            r"C:\Users\evank\Documents\ArcGIS\Projects\Temperature\cru_ts4.04.1901.2019.dtr.dat.nc\cru_ts4.04.1901.2019.dtr.dat.nc",
            "dtr", "lon", "lat", "dtr_" + str(year_number) + "_" + str(month_number), '',
            "time '" + str(month_number) + "/01/" + str(year_number) + " 12:00:00 AM'", "BY_VALUE", "CENTER")
    # AVERAGES THE RASTERS then deletes extra rasters
    inRaster = ["dtr_" + str(year_number) + "_" + month_number for month_number in month]
    # Execute Times
    Year_DTR_Average = (Raster(inRaster[0]) + Raster(inRaster[1]) + Raster(inRaster[2]) + Raster(inRaster[3]) +
                    Raster(inRaster[4]) +
    ...
    # Save the output
    arcpy.management.ProjectRaster(Year_DTR_Average,
                                   r"C:/Users/evank/Documents/ArcGIS/Projects/Temperature/Temperature Variable CRU/DTR/Year_DTR_Average" + str(year_number) + ".tif",
    ...
    out_point_features = (
                r"C:/Users/evank/Documents/ArcGIS/Projects/Temperature/Temperature Variable CRU/DTR/Specimen_Location_" +
                str(year_number))
    ExtractValuesToPoints(in_point_features, "Year_DTR_Average" + str(year_number) + ".tif", out_point_features, "NONE", "Value_ONLY")
# Export the saved file as a .csv file
    import numpy
    fc = (r"C:/Users/evank/Documents/ArcGIS/Projects/Temperature/Temperature Variable CRU/DTR/Specimen_Location_" +
          str(year_number) + ".shp")
    ...
    for month_number in month:
        arcpy.Delete_management("dtr_" + str(year_number) + "_" + month_number)
    arcpy.Delete_management("Year_DTR_Average" + str(year_number) + ".tif")
